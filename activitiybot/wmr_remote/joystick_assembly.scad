...
joystick_clr = 1;
joystick_hole_radius = 3.5/2-0.2;
pcbjoystickgap = 2;
...
module base(H=2*wall_thickness+12){ // Base surrounding ESP module
...
module joy_base(){ // Joystick base 
	color("red")translate([-joystick_L/2-pcbjoystickgap,-joystick_W/2])square([joystick_L+pcbjoystickgap+0.2,joystick_W]);
}
...
difference(){
	union(){
		...
		translate([-joystick_L-pcb_L/2-pcbjoystickgap,-joystick_W/2,wall_thickness])linear_extrude(height=5,convexity=10)joystick_hole_plan();
		//translate([-35,-15,2])linear_extrude(height=3,convexity=10)pcb_hole_plan();
		bod();
	}
	usb_cutout();
	// filament saving cutout in center:
	//translate([-65,-joystick_W/4,-1])linear_extrude(height=wall_thickness+20,convexity=10)square([100,joystick_W/2]);
	// drill out the holes
	//translate([-33.3-35,-26.69/2,-1])linear_extrude(height=21,convexity=10)joystick_hole_plan();
	//translate([-35,-15,-1])linear_extrude(height=21,convexity=10)pcb_hole_plan();
	L = 70+4+33.3+50;
...
base(H=3);
	...
	translate([35-5+5,-5-1,-0.1])linear_extrude(height=5.3,convexity=10)square([10,12]);
...
module pcb_hole_plan(hole_radius=pcb_hole_radius,clr=2.5){
		//clr=1;//clearance
		//hole_radius=3.5/2+0.1;
		holesep_x = pcb_L-2*clr;
		holesep_y = pcb_W-2*clr;
		translate([clr,clr])circle(r=hole_radius);  
		translate([clr+holesep_x,clr])circle(r=hole_radius);  
		translate([clr,clr+holesep_y])circle(r=hole_radius);  
		translate([clr+holesep_x,clr+holesep_y])circle(r=hole_radius); 
}
